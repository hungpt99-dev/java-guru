#!/usr/bin/env bash
# 4th pass: aggressive retry of the 4 still-missing FinPay AI rewrites.
# Up to 5 attempts each; rotates temp dir; falls back to single-locale if combined keeps failing.
set -uo pipefail
cd /root/java-guru
...
for N in 02 03 06 08; do
  ...
  for attempt in 1 2 3 4 5; do
    TMP="/tmp/fp4_${N}_${SLUG_N}_a${attempt}"
  ...
  if [ "$done_this" -eq 0 ]; then
    echo "STILL MISSING after 5 attempts: ai/$SLUG_N"
  else
    git add -A
    git commit -q -m "rewrite(finpay-ai): $SLUG_N (retry pass 4)" 2>&1 | tail -1
    git push origin main 2>&1 | tail -1
    echo "--- pushed $SLUG_N ---"
  fi
done
echo "FINPAY AI 4TH PASS DONE"
